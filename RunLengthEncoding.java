...
//Two phases, insertion and consolidation
          ...
          if (sumTrack+curr.number>point) {
              if (curr.type==EMPTY) {
                  ...
                  if (combine.prev!=null) {
                      if (combine.prev.type==combine.type) {
                          int total = combine.number+combine.prev.number;
                          oceanNode newNode = new oceanNode(SHARK,total,0);
                          newNode.next = combine.next;
                          newNode.prev = combine.prev.prev;
                          combine.prev.prev.next = newNode;
                          combine.next.prev = newNode;
  ...
  private void check() {
      ...
      while (curr.next!=null) {
          if (curr.type==curr.next.type && curr.type!=SHARK) {
              System.out.println("Run is illegal;consectutive of same type");
          }
          if (curr.type==curr.next.type && curr.type==SHARK) {
              if (curr.hunger==curr.next.hunger) {
                  System.out.println("Run is illegal;consecutive sharks of same type");
                  System.out.println("First "+curr.hunger+" and second "+curr.next.hunger);
              }
          }
          if (curr.number==0) {
              System.out.println("Run is illegal;length is 0");
          }
          runLength+=curr.number;
          curr=curr.next;
      }
      runLength+=curr.number;
      if (runLength!=xDimStore*yDimStore) {
          System.out.println("Run is illegal;length is not equal");
          System.out.println("curr Run "+runLength+" grid"+(xDimStore*yDimStore));
      }
  }
