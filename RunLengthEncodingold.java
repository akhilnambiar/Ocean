...
/*      System.out.println("working");
      ...
      if (runBegin==head && hasLooped==true) {
          return null;
      }
...
/*      Ocean newOcean = new Ocean(xDimStore,yDimStore,starveTime);
  ...
  public RunLengthEncoding(Ocean sea) {
    ...
    //Write an insert beginning method
      ...
      for (int i=0; i<xDimStore; i++) {
          for (int j=0; j<yDimStore; j++) {     //Sharks need a special check
              if (i==0 && j==0) {
                  i++;
              }
              if (sea.cellContents(i,j)==curType) {
                  if (curType==SHARK) {
                      if (sea.sharkFeeding(i,j)==curHung) {
                          ...
                          oceanNode newNode = new oceanNode(curType,curHung);
                          insertFront(newNode);
                  ...
                  oceanNode newNode = new oceanNode(curType,runTrack);
                  insertFront(newNode);
                  curType = sea.cellContents(i,j);
                  runTrack=1;
                  if (curType==SHARK) {
